...
def interpolate_and_normalize(image, slice_thickness, pixel_spacing, norm=True):

    # # clip values
    # image = np.clip(image, a_min=-1200, a_max=600) 

    # interpolate slices
    image = ndimage.zoom(image, (slice_thickness, pixel_spacing[0], pixel_spacing[1]))
    ...
    image = [
        cv.resize(im, dsize=(256, 256))
        for im in image
    ]

    # return middle 8 slices
    start = max(0, int(len(image)/2) - 4)
    stop = start + 8
    return np.array(image[start:stop])
