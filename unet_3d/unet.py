...
def encoder_block(x, filters, kernel_size, downsample=False):
    ...
    # Convolve
    x = Conv3D(filters, kernel_size, **conv_kwargs)(x)
    x = Conv3D(filters, kernel_size, **conv_kwargs)(x)
    return x
...
def decoder_block(inputs, filters, kernel_size, transpose=True):
    ...
    if transpose:
        ...
        up = Conv3DTranspose(filters, (2, 2, 1), strides=(2, 2, 1), **conv_kwargs)(x)
    else:
        # Upsampling by simply repeating rows and columns then convolve
        up = UpSampling3D(size=(2, 2, 1))(x)
        up = Conv3D(filters, 2, **conv_kwargs)(up)

    # Concatenate u-net shortcut to input
    x = concatenate([shortcut, up], axis=4)
...
def unet(input_size=(256, 256, 4, 1), output_channels=1, transpose=True):
    """
    ...
    e1 = encoder_block(inputs, 64, (2, 2, 2), downsample=False)
    e2 = encoder_block(e1, 128, (2, 2, 2), downsample=True)
    e3 = encoder_block(e2, 256, (2, 2, 2), downsample=True)
    e4 = encoder_block(e3, 512, (2, 2, 2), downsample=True)
    e4 = Dropout(0.5)(e4)

    e5 = encoder_block(e4, 1024, (2, 2, 2), downsample=True)
    e5 = Dropout(0.5)(e5)

    d6 = decoder_block([e5, e4], 512, (2, 2, 2), transpose=transpose)
    d7 = decoder_block([d6, e3], 256, (2, 2, 2), transpose=transpose)
    d8 = decoder_block([d7, e2], 128, (2, 2, 2), transpose=transpose)
    d9 = decoder_block([d8, e1], 64,  (2, 2, 2), transpose=transpose)

    # Ouput
    op = Conv3D(2, 3, padding='same', kernel_initializer='he_normal')(d9)
    op = ReLU()(op)
    op = Conv3D(output_channels, 1)(op)
